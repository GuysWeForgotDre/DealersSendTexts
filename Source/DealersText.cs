...
namespace DealersSendTexts
{
    ...
    public class DealerPrefs
    {
        ...
        private static readonly Dictionary<string, DealerPrefs> DealerSettings = new Dictionary<string, DealerPrefs>();
        ...
        public static DealerPrefs Prefs(string firstName) => MasterOnly ? Master : DealerSettings.TryGetValue(firstName, out var prefs) ? prefs : AddDealer(firstName);
        private static DealerPrefs AddDealer(string name)
        {
            ...
            prefs.IsStuckCount.Comment   = "Number of failed checks to send stuck alert (every 30 min) (default 4)";
            ...
            prefs.DealerGroup.SetFilePath(ModSaveData.FullPath(config: true));
